...
from models import FeeConfig, FeePayment, User
...
def admin_fee_dashboard():
    if current_user.role!="Admin":
        ...
        return redirect(url_for("student_fee.student_fees"))  # student fallback if not admin
...
def dropdowns():
    ...
    progs=sorted({u.program for u in User.query.filter(User.role=="Student", User.program!=None)})
    branches=sorted({u.branch for u in User.query.filter(User.role=="Student", User.branch!=None)})
    years=sorted({u.year for u in User.query.filter(User.role=="Student", User.year!=None)})
...
def save_fee_config():
    ...
    try:
        cfg=FeeConfig(program=data.get("program","").upper(),
...
def students():
    ...
    if mode == "academic":
        ...
        if program: query = query.filter(User.program == program)
        if branch: query = query.filter(User.branch == branch)
        if year: query = query.filter(User.year == year)
        students = query.all()
    ...
    for st in students:
        def stats(ftype):
            ...
            paid = sum(float(p.amount) for p in st.fee_payments if p.status == "Paid" and p.fee_type == ftype)
...
def manage_accounts():
    ...
    if request.method == "POST":
        ...
        if acc.is_default:
            ...
            InstitutionAccount.query.filter_by(fee_type=acc.fee_type).update({"is_default": False})
...
def reports():
    ...
    by_fee_type = db.session.query(FeePayment.fee_type, db.func.count(FeePayment.id), db.func.sum(FeePayment.amount)).group_by(FeePayment.fee_type).all()
    by_account = db.session.query(InstitutionAccount.account_name, InstitutionAccount.account_type, FeePayment.status,
                                  db.func.count(FeePayment.id), db.func.sum(FeePayment.amount))\
                            .join(FeePayment, InstitutionAccount.id==FeePayment.account_id)\
                            .group_by(InstitutionAccount.account_name, FeePayment.status).all()
    return render_template("admin_reports.html", by_fee_type=by_fee_type, by_account=by_account)
...
def payments(sid):
    ...
    return jsonify([{"id":p.id,"amount":float(p.amount),"status":p.status,
                     "created_at":p.created_at.strftime("%Y-%m-%d"),
                     "fee_type":p.fee_type} for p in st.fee_payments])