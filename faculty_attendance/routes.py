...
def faculty_attendance():
    ...
    branches = [
        b[0]
        for b in db.session.query(User.branch)
    ...
    if request.method == "POST" and "load_students" in request.form:
        ...
        if selected_branch:
            students = User.query.filter_by(
                branch=selected_branch, role="Student"
            ).all()
...
def save_attendance():
    ...
    for student in students:
        ...
        existing = Attendance.query.filter_by(
            student_id=student.id, date=selected_date_obj, branch=selected_branch
        ).first()
    ...
    return redirect(
        url_for(
            "faculty_stud_bp.faculty_attendance",
            branch=selected_branch,
            date=selected_date,
        )
    )
